<template>
  <div class="home">
    ...
    <div class="card card2">
      <live-player
        video-url="http://192.168.20.225:10800/flv/hls/stream_2.flv"
        :live="true"
        :stretch="true"
    ...
    <div class="card card4">
      ...
      <div class="dashboard">
        ...
        <div class="bottom">行车里程(hm)</div>
      </div>
      <div class="dashboard right">
        <div id="chart3" class="chart">
          <div class="digital" v-for="(item,index) in (ms+'').length" :key="item">{{(ms+'')[index]}}</div>
        </div>
        <div class="plate">节约时间(s)</div>
      </div>
      <div class="dashboard right" style="margin-right:0">
        <div id="chart3" class="chart">
          <div class="digital" v-for="(item,index) in (bd+'').length" :key="item">{{(bd+'')[index]}}</div>
        </div>
        <div class="plate">优化次数</div>
      </div>
    </div>
...
<script>
import file from '../../../static/km.json'
...
export default {
  ...
  mounted() {
    ...
    // this.writeFile()
    // file.km = 20
    this.km = localStorage.getItem('km') ? localStorage.getItem('km') : 0
    // require('../../../static/km.json') = { km: 30 }
  ...
  methods: {
    readFile() {
      this.$http.get('../../../static/km.txt').then(res => {
        this.km = res.data
      })
    },
    writeFile() {
    ...
    initSignalInfo() {
      this.socketInfo = new SockJS(`${this.baseUrl}/webSocketServer`)
      this.stompClientInfo = Stomp.over(this.socketInfo)
      console.log('启动前时间：' + new Date().toLocaleTimeString())
      this.stompClientInfo.connect(
        {},
        frame => {
          console.log('建立连接成功aaa')
          const subscribeUrl = `/user/aaa/countAndTime`
          this.stompClientInfo.subscribe(subscribeUrl, res => {
            const re = JSON.parse(res.body)
            this.ms = re.saveTime
            this.bd = re.reserveCount
            console.log(res)
            // this.ms = re.
            // var light = re.keyList[2]
          })
        ...
        frame => {
          ...
          this.stompClientSig.subscribe(subscribeUrl, res => {
            ...
            re.keyList.forEach(element => {
              if (element.index == this.currentPhase) {
        ...
        frame => {
          ...
          this.carStompClientSig.subscribe(subscribeUrl, res => {
            ...
            if (re.type === 69) {
              ...
              if (this.speed >= 0.1) {
                this.km = (
                  (this.km * 10 + (parseFloat(this.speed) * 10 * 0.5) / 100) /
                  10
                ).toFixed(3)
                localStorage.setItem('km', this.km)
              }
              this.initCar()
              var x = re.lon
              var y = re.lat
              var d = re.direction
              // var ds = (x - 10) * (x - 10) + (y - 10) * (y - 10)
              if (re.phaseCode == 2) {
                if (this.currentPhase == 10) {
                  this.tipsFlag = 10
                  this.word = '前方绿波路段，切换一号灯组'
                  setTimeout(() => {
                    this.tipsFlag = false
                  }, 4000)
                }
                this.currentPhase = 2
              } else if (re.phaseCode == 10) {
...
.home {
  ...
  .card2 {
    top: 813.5px;
    left: 23px;
    background: url('../../assets/carAndRoad/card-people.png');
  ...
  .card4 {
    ...
    top: 813.5px;
    padding-left: 42.5px;
    background: url('../../assets/carAndRoad/card5.png');
    .title {
      width: 65px;
      height: 13px;
      background: url('../../assets/carAndRoad/title.png');
      margin-left: -20px;
    }
    .dashboard {
      width: 270px;
      ...
      #chart2 {
        background: url('../../assets/carAndRoad/chart5.png');
        background-size: 100% 100% !important;
        .value {
          font-size: 64px;
          font-family: 'UnidreamLED';
      ...
      #chart3 {
        .digital {
          ...
          height: 73px;
          line-height: 73px;
          background: url('../../assets/carAndRoad/digital.png');
          background-size: 100% 100% !important;
          font-size: 50px;
          color: #fff;
          font-weight: bold;
      ...
      .plate {
        width: 312px;
        height: 71px;
        background: url('../../assets/carAndRoad/plate.png');
        line-height: 100px;
        font-size: 16px;
        text-align: center;
        color: #fff;
