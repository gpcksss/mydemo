<template>
  <div class="hello">
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column align="left" prop="nickName" label="员工姓名" width="200"></el-table-column>
      <el-table-column align="left" prop="weekdate" label="周日期" width="300"></el-table-column>
      <el-table-column align="left" label="周工作内容">
...
export default {
  ...
  mounted() {
    var date = new Date()
    var num = date.getDay() === 0 ? 13 : date.getDay() + 6
    var lastWeekStart = date.getTime() - 86400000 * num
    var date2 = new Date(lastWeekStart + 86400000)
    var month =
      date.getMonth() + 1 > 9
        ? date.getMonth() + 1
        : '0' + (date.getMonth() + 1)
    var day = date.getDate() > 9 ? date.getDate() : '0' + date.getDate()
    this.searchObj.date = date.getFullYear() + '-' + month + '-' + day
    this.changeWeek(date2)
  ...
  methods: {
    ...
    submit() {
      ...
      var endDate = ''
      if (this.dateType === 0) {
        startDate = this.weekFormat.split(' ~ ')[0]
        endDate = this.weekFormat.split(' ~ ')[1]
      } else if (this.dateType === 1) {
        startDate = this.searchObj.date + '-01'
        endDate = this.searchObj.date + '-31'
      }
      this.$http
        .get('http://39.98.187.79:9048/ruoyi/app/week_log/weblist', {
          params: {
